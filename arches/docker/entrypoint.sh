...
cd_supervisor() {
	cd ${SUPERVISOR}
	echo "Current work directory: ${SUPERVISOR}"
}
...
run_celery(){
	...
	if [[ -z ${ARCHES_PROJECT} ]]; then
		supervisord -c /etc/supervisor/arches-supervisord.conf
	else
		cd_supervisor
		if [[ !  -z ${ARCHES_PROJECT}-supervisord.conf ]]; then
				mv arches-supervisord.conf ${ARCHES_PROJECT}-supervisord.conf  
	    		sed -i 's/arches-celeryd.conf/'${ARCHES_PROJECT}'-celeryd.conf/g' ${ARCHES_PROJECT}-supervisord.conf
		fi
		cd /etc/supervisor/conf.d
		if [[ ! -z ${ARCHES_PROJECT}-celerybeat.conf ]]; then
				mv arches-celerybeat.conf ${ARCHES_PROJECT}-celerybeat.conf
				sed -i 's/arches.celery/'${ARCHES_PROJECT}'.celeryw/g' ${ARCHES_PROJECT}-celerybeat.conf
				sed -i '7s/arches/'${ARCHES_PROJECT}'/g' ${ARCHES_PROJECT}-celerybeat.conf
		fi
    	if [[  ! -z ${ARCHES_PROJECT}-celeryd.conf ]]; then 
			mv arches-celeryd.conf ${ARCHES_PROJECT}-celeryd.conf
			sed -i 's/arches.celery/'${ARCHES_PROJECT}'.celeryw/g' ${ARCHES_PROJECT}-celeryd.conf
			sed -i '7s/arches/'${ARCHES_PROJECT}'/g' ${ARCHES_PROJECT}-celeryd.conf
		fi
		supervisord -c /etc/supervisor/${ARCHES_PROJECT}-supervisord.conf	
	fi	
	
	echo "----- *** CHECKING IF SUPERVISOR IS RUNNING AS A DAEMON *** -----"
	tail /var/log/supervisor/supervisord.log
	echo "----- *** CHECKING IF CELERY IS RUNNING *** -----"
	sleep 3
